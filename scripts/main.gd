...
func _ready() -> void:
	...
	if FileAccess.file_exists("user://directory.pmlconfig"):
		...
		if !DirAccess.dir_exists_absolute(ptdir) || (!FileAccess.file_exists(ptdir + "/PolyTrack.exe") && !FileAccess.file_exists(ptdir + "/PolyTrack")):
			$loader/polydir.visible = true
		$loader/directory.text = ptdir
		if !DirAccess.dir_exists_absolute(ptdir + "/mods"):
			setup_mod_environment(ptdir)
		reload_mods()
	else:
		$loader/polydir.visible = true
	cmenu = "loader"
...
func setup_mod_environment(dir: String) -> void:
	...
	if !polydir.dir_exists("mods"):
		...
		add_child(http)
		var reqcompleted: Callable = \
		# use on completed request
		func _request_completed(result:int, _response_code:int, _headers:PackedStringArray, _body:PackedByteArray):
			if result != OK:
		...
		# connect signal and attempt download
